...
If both sides changed, expand the pair and compare the device and Nextcloud type, size, and modification details before choosing an action. **Keep both copies** is the conservative recovery. When every conflict on the displayed page allows the same choice, the matching **all** action can submit that reviewed page as one atomic batch. The app checks every selected generation again before saving anything; if one item changed, none of the batch runs and the page returns to review. Additional conflicts are loaded as earlier pages are resolved.
