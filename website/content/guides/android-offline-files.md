...
nati.ve distinguishes a server listing, a cached preview, an original stored offline, and a synchronized folder. This guide is for individual files and folders you explicitly keep on the Android device. Use the separate Android folder-sync guide when a normal device folder must exchange changes in both directions.

## 1. Make the required files available offline

@capture-alt: nati.ve Android Files workspace showing cloud folders and documents with favorites, previews, transfer state, and offline availability indicators
...
Open **Files**, browse to an item, open its menu, and choose **Make available offline**. For a folder, nati.ve plans the recursive contents before downloading them. Keep the app online until the item reports that it is available. A queued, downloading, or waiting-for-network state is not offline readiness.
...
@capture-alt: nati.ve Android virtual-file storage view showing the System Files provider, cache usage, protected offline pins, free space, and cleanup policy
...
Open **Settings**, choose **Sync & offline**, and review **Virtual files**. nati.ve is exposed automatically under **System Files / nati.ve** while the account is signed in. Opening a remote item through another Android app hydrates a complete cached generation. That ordinary cache may be reclaimed later; an explicit offline pin is a stronger retention class.
...
@capture-alt: nati.ve Android transfer view with pending, active, failed, and completed work separated into actionable states
...
For a failed download, check connectivity, server permissions, available device space, and whether the remote version changed. Retry from the visible failure state. nati.ve writes to staging storage and validates a complete generation before publishing it as available, so a partial transfer should never be presented as a finished offline file.
