    ...
    return buildSet {
        ...
        recordedDocumentIds.forEach { documentId ->
            val reference = runCatching { NextcloudDocumentIds.parse(documentId) }.getOrNull()
                ?: return@forEach
            if (reference.accountKey != root.accountKey || reference.incarnation != root.incarnation) return@forEach
        ...
        AndroidSafOwnedDownloadRecoveryDirectory(
            documentId = NextcloudDocumentIds.documentId(root.accountKey, root.incarnation, fullPath),
...
internal fun androidSafOwnedDownloadRecoveryDirectory(
    ...
    if (directory.accountKey != root.accountKey || directory.incarnation != root.incarnation) return null
    val relativePath = when {
...
internal fun reconcileOwnProviderSafDownloadsBeforePairRemoval(
    ...
    if (androidRootBoundProviderRecoveryIdentity(root, providerRecoverySession?.let { androidProviderRecoveryIdentity(context, it) }) != null) {
        ...
        withAndroidCrossAccountProviderRecovery(root, tree.authority == nextcloudDocumentsAuthority(context.packageName), services::loadSession,
            matchesRoot = { androidRootBoundProviderRecoveryIdentity(root, androidProviderRecoveryIdentity(context, it)) != null }) { session ->
...
private fun reconcileBoundProviderSafDownloads(
    ...
    val recoveryIdentity = providerRecoverySession?.let { androidProviderRecoveryIdentity(appContext, it) }
    val discoveryRootId = androidSafRetirementDiscoveryIdentityRoot(
        DocumentsContract.getTreeDocumentId(treeUri), recoveryIdentity.takeUnless { preserveTreeGrant },
    )
    ...
    val localTree = AndroidSafFileSyncLocalTree(
        ...
        providerRecoveryIdentity = recoveryIdentity,
    )
...
internal fun androidSafRetirementDiscoveryRoot(rootDocumentId: String, session: NextcloudSession?): String =
    androidRootBoundProviderRecoverySession(rootDocumentId, session)?.let { NextcloudDocumentIds.rootId(it, NextcloudDocumentIncarnation.Legacy) }
        ?: rootDocumentId

internal fun androidSafRetirementDiscoveryIdentityRoot(rootDocumentId: String, identity: AndroidProviderRecoveryIdentity?): String =
    androidRootBoundProviderRecoveryIdentity(rootDocumentId, identity)?.let {
        val root = NextcloudDocumentIds.parse(rootDocumentId)
        NextcloudDocumentIds.rootId(root.accountKey, root.incarnation)
    } ?: rootDocumentId
