...
internal class AndroidFileSyncCapabilityLifecycle internal constructor(
    ...
    fun acquire(
        ...
        exactUri: String,
        displayName: String,
        generation: AndroidFileSyncRootAcquisitionGeneration = AndroidFileSyncRootAcquisitionGenerations.capture(accountId),
    ): FileSyncLocalRoot = synchronized(LIFECYCLE_LOCK) {
        AndroidFileSyncRootAcquisitionGenerations.requireCurrent(accountId, generation)
    ...
    fun reconcile(state: AndroidFileSyncPersistedState, reclaimUnrestoredReady: Boolean = false, retiringAccount: AndroidFileSyncCapabilityAccountId? = null) = synchronized(LIFECYCLE_LOCK) {
        ...
        records.forEach { original -> attemptRecovery {
            ...
            when (record.phase) {
                AndroidFileSyncCapabilityPhase.Acquiring -> if ((retiringAccount != null && record.accountId == retiringAccount && matchingIds.isNotEmpty()) || record.processGeneration != processGeneration || reclaimUnrestoredReady) {
                ...
                AndroidFileSyncCapabilityPhase.Ready -> if ((retiringAccount != null && record.accountId == retiringAccount && matchingIds.isNotEmpty()) || record.processGeneration != processGeneration || record.id in abandonedSelections ||
    ...
    fun retireAccountSetup(
        ...
        AndroidFileSyncRootAcquisitionGenerations.retire(accountId)
    ...
    private fun reconcileSetup(
        ...
        reconcile(state, retiringAccount = accountId.takeIf { includeCurrentGeneration })
        ...
        records.asSequence()
            .filter { record ->
                record.accountId == accountId &&
                    (record.phase == AndroidFileSyncCapabilityPhase.Ready ||
                        (includeCurrentGeneration && record.phase == AndroidFileSyncCapabilityPhase.Acquiring)) &&
