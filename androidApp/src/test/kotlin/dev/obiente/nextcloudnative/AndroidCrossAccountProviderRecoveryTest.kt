...
class AndroidCrossAccountProviderRecoveryTest {
    ...
    private val root = NextcloudDocumentIds.rootId(session, NextcloudDocumentIncarnation.Legacy)
    ...
    @Test fun canonicalAccountLeaseAlsoBlocksCrossAccountRecovery(): Unit = runBlocking {
        val guard = AndroidAccountOperationGuard()
        guard.withAccount(session.accountId.storageKey) {
            assertFailsWith<IllegalStateException> {
                withAndroidCrossAccountProviderRecovery(root, true, { session }, guard) { error("Busy canonical identity must retain pending work") }
            }
        }
        assertEquals(session, withAndroidCrossAccountProviderRecovery(root, true, { session }, guard) { it })
    }
