...
class NextcloudDocumentsContractTest {
    ...
    fun `recovery permit resolves only its inactive target operation`() {
        ...
        val removedDocument = NextcloudDocumentIds.documentId(removed, NextcloudDocumentIncarnation.Legacy, "Sync/report.txt")
        val activeDocument = NextcloudDocumentIds.documentId(active, NextcloudDocumentIncarnation.Legacy, "Documents/current.txt")
    ...
    fun `recovery permit is confined to its synchronous dispatch thread`() {
        ...
        val documentId = NextcloudDocumentIds.documentId(unavailable, NextcloudDocumentIncarnation.Legacy, "Sync")
    ...
    fun `external provider caller cannot consume an inactive recovery permit`() {
        val active = session("active", "active-secret")
        val removed = session("removed", "removed-secret")
        val removedDocument = NextcloudDocumentIds.documentId(removed, NextcloudDocumentIncarnation.Legacy, "Sync")
    ...
    fun `paused recovery rejects unrelated mutation and writable open`() {
        val active = session("active", "active-secret")
        val removed = session("removed", "removed-secret")
        val recoveryDocument = NextcloudDocumentIds.documentId(removed, NextcloudDocumentIncarnation.Legacy, "Sync/.nextcloud-native-stage")
        val unrelatedDocument = NextcloudDocumentIds.documentId(removed, NextcloudDocumentIncarnation.Legacy, "Sync/private.txt")
    ...
    fun `read descriptor session survives its consumed permit without enabling write open`() {
        val removed = session("removed", "removed-secret")
        val documentId = NextcloudDocumentIds.documentId(removed, NextcloudDocumentIncarnation.Legacy, "Sync/recovery-backup")
    ...
    fun `recovery uri helpers use direct provider calls and deny unsupported operations`() {
        ...
        val documentId = NextcloudDocumentIds.documentId(session, NextcloudDocumentIncarnation.Legacy, "Sync")
    ...
    fun `recovery permit is cleared when recovery fails`() {
        val removed = session("removed", "removed-secret")
        val documentId = NextcloudDocumentIds.documentId(removed, NextcloudDocumentIncarnation.Legacy, "Sync")
