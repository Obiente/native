...
class AndroidProviderRecoverySafetyTest {
    ...
    fun relocatedOwnedFilesAreDiscoverableOutsideTheOldSubtreeOnlyWithBoundAccountAccess() {
        ...
        val oldRoot = NextcloudDocumentIds.documentId(session, NextcloudDocumentIncarnation.Legacy, "original/subtree")
        val movedDirectory = NextcloudDocumentIds.documentId(session, NextcloudDocumentIncarnation.Legacy, "elsewhere/moved")
        val discoveryRoot = androidSafRetirementDiscoveryRoot(oldRoot, session)
        kotlin.test.assertEquals(NextcloudDocumentIds.rootId(session, NextcloudDocumentIncarnation.Legacy), discoveryRoot)
        ...
        kotlin.test.assertNull(androidSafOwnedDownloadRecoveryDirectory(
            discoveryRoot, NextcloudDocumentIds.documentId(session.copy(loginName = "bob"), NextcloudDocumentIncarnation.Legacy, "elsewhere/moved"),
        ))
    }
    ...
    fun legacyTreeRecoveryBindsOnlyTheAccountOwningItsDocument() {
        ...
        kotlin.test.assertEquals(removing, androidRootBoundProviderRecoverySession(NextcloudDocumentIds.rootId(removing, NextcloudDocumentIncarnation.Legacy), removing))
        kotlin.test.assertNull(androidRootBoundProviderRecoverySession(NextcloudDocumentIds.rootId(other, NextcloudDocumentIncarnation.Legacy), removing))
        kotlin.test.assertNull(androidRootBoundProviderRecoverySession(NextcloudDocumentIds.rootId(other, NextcloudDocumentIncarnation.Legacy), null))
