...
- [Android] Direct APK installations can now choose between signed Alpha and Nightly update channels while preserving channel, version, checksum, package, and signer validation (issue #228, PR #229).
...
- [Android, Desktop] Files can now sync selected folders either way with filters and priority rules, recover safely in the background, and expose writable virtual files on Android, Linux, and Windows. A live desktop tray shows progress and conflicts (issue #11).
...
- [Android, Desktop, Linux, macOS, Windows] Nightly builds now publish signed Android packages and native desktop packages, with selectable Alpha and Nightly update channels on Android (issue #226, PR #227).
...
- [Android, Desktop, Linux, macOS, Windows] Nightly publication now creates a missing immutable tag correctly instead of mistaking GitHub's not-found response for a tag collision (issue #226).
- Nightly releases now explain the user-facing features and fixes in each build with platform and issue context (issue #269).
- [Android, Desktop, Linux, macOS, Windows] Nightly releases now verify repository tooling from the tested source, include nested Linux packages, and count only platforms represented by staged release assets (issue #226).
