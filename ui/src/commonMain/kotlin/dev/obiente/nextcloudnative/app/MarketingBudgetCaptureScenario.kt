...
internal fun MarketingBudgetDynamicWorkspaceScenario(scenario: MarketingCaptureScenario) {
    ...
    require(
        accounts || scenario == MarketingCaptureScenario.BudgetTransactionsDesktop ||
            scenario == MarketingCaptureScenario.BudgetTransactionsMobile,
    ) { "${scenario.id} is not a Budget dynamic workspace capture." }
    val desktop = scenario.presentation == dev.obiente.nextcloudnative.app.design.NextcloudPresentation.Desktop
    val selectedView = if (accounts) marketingBudgetAccountsView else marketingBudgetTransactionsView
    val records = if (accounts) marketingBudgetAccountRecords else marketingBudgetTransactionRecords
    val destinations = listOf(
    ...
    NextcloudCollectionWorkspaceScaffold(
        ...
        contentSubtitle = if (accounts) "Balances and account health" else "Search, review and categorize activity",
        onBack = {},
        hasHierarchyBack = false,
        onDestinationSelected = {},
    ) {
