...
class DesktopAccountCleanupRecoveryTest {
    @Test
    fun abortedRemovalReopensDiskContractsButRejectsOldProducers() = runBlocking {
        val root = Files.createTempDirectory("desktop-cleanup-cache-test").toFile()
        val preferences = Preferences.userRoot().node("desktop-cleanup-cache-test-${UUID.randomUUID()}")
        val session = NextcloudSession("https://cloud.example.test", UUID.randomUUID().toString(), "password")
        val accountKey = session.accountId.storageKey
        val cacheKey = desktopFileCacheAccountId(session)
        val discovery = DesktopDynamicDiscoveryCache(root.resolve("contracts"))
        val files = DesktopFileReadCache(root.resolve("files"), preferences = preferences)
        val oldProducer = sharedDynamicNativeMemoryCache.producer(session)
        try {
            discovery.save(accountKey, cacheKey, "notes", "old", oldProducer)
            AccountPrivateMemoryLifecycle.retireAccount(accountKey)
            discovery.fenceAccount(accountKey)
            files.retireAccount(cacheKey)
            retryDesktopAccountSyncPairCleanup(
                cleanup = DesktopAccountSyncPairCleanup(
                    cacheKey, DesktopAccountSyncPairCleanupPhase.Prepared, MUTATION_SCOPE, accountKey,
                ),
                accountOwnership = { DesktopAccountOwnership.Present },
                removeSyncPairs = { error("Retained credentials must preserve account data") },
                clearCleanup = {},
                reactivatePresentAccount = { reactivateDesktopAccountPrivateCaches(it, discovery, files) },
            )
            assertEquals("old", discovery.load(accountKey, cacheKey, "notes"))
            val freshProducer = sharedDynamicNativeMemoryCache.producer(session)
            discovery.save(accountKey, cacheKey, "notes", "fresh", freshProducer)
            discovery.save(accountKey, cacheKey, "notes", "stale", oldProducer)
            assertEquals("fresh", discovery.load(accountKey, cacheKey, "notes"))
            assertTrue(files.producer(cacheKey) != null)
        } finally {
            AccountPrivateMemoryLifecycle.retireAccount(accountKey)
