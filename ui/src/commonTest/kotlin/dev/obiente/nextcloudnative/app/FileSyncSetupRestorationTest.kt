...
class FileSyncSetupRestorationTest {
    ...
    @Test
    fun unavailableRestoredPermissionAbandonsItsOpaqueReference() = runBlocking {
        ...
        kotlin.test.assertFalse(restoreAndReconcileFileSyncRootSetup(draft,
            restore = { null }, reconcile = { error("Unavailable permission cannot be reconciled as retained") },
            abandon = { abandoned += it; true },
        ))
        assertEquals(listOf(reference), abandoned)
    }

    @Test
    fun unavailableStaleRestorationAbandonsOnlyItsOwnOpaqueReference() = runBlocking {
        val draft = FileSyncSetupDraftState().apply { localRoot.value = reference }
        val replacement = restored.copy(savedStateId = "replacement-id")
        val abandoned = mutableListOf<FileSyncLocalRoot>()
        assertTrue(restoreAndReconcileFileSyncRootSetup(draft,
            restore = { draft.localRoot.value = replacement; null },
            reconcile = { error("A stale restoration cannot reconcile the replacement") },
            abandon = { abandoned += it; true },
        ))
